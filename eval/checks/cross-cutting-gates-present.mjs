...
export const description =
  'Each user-facing skill must reference forge cross-cutting gates: Forbidden language (forge) and inline-gloss / communication-style (all user-facing skills).';
...
const SUPPLEMENTARY = ['lock', 'launch', 'strike', 'tag'];
const SUPP_REQUIRED = [
...
export async function checkRepo(rootDir) {
  ...
  // forge
  const forgePath = path.join(rootDir, 'skills', 'forge', 'SKILL.md');
  const forgeContent = await fs.readFile(forgePath, 'utf8');
  for (const req of FORGE_REQUIRED) {
    if (!forgeContent.includes(req.needle)) {
      findings.push({
        severity: 'fail',
        message: `skills/forge/SKILL.md missing ${req.name} (needle: "${req.needle}")`,
      });
    }
  }
